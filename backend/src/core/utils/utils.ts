...
export const encryptData = (data: string): string => {
  ...
  const cipher = crypto.createCipheriv(ALGORITHM, Buffer.from(ENCRYPTION_KEY, 'utf-8'), null);
...
export const decryptData = (encryptedData: string): string => {
  ...
  try {
    const decipher = crypto.createDecipheriv(ALGORITHM, Buffer.from(ENCRYPTION_KEY, 'utf-8'), null);
    let decrypted = decipher.update(encryptedData, 'hex', 'utf-8');
    decrypted += decipher.final('utf-8');
    return decrypted;
  } catch (error) {
    throw new Error('Decryption failed: possibly due to an incorrect key or corrupted data'); // Handle decryption failure
  }
}

// Verification function to check if decrypted data matches original data
export const verifyEncryptedData = (originalData: string, encryptedData: string): boolean => {
  try {
...
export const verifyHashedData = async (data: string | null, hashedData: string | null): Promise<boolean> => {
