...
const getMainCategories = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Query the categories based on `categoryIds` or `categoryNumArr`
    const categories = await prisma.category.findMany({
      where: {
        OR: [{ id: { in: categoryIds } }, { categoryNumber: { in: categoryNumArr } }],
      },
      include: {
        parentCategory: true, // Include parent category details
      },
    })

    // Find the main categories (parent/root categories)
    const mainCategoryIds = categories
      .map((category) => category.parentId || category.id) // Fallback to `id` if no parentId
      .filter((id, index, self) => self.indexOf(id) === index) // Remove duplicates

    // Fetch the main categories by their IDs
