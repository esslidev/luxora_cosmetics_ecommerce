  ...
  try {
    ...
    const formats = product.formats.map((format) => {
      ...
      return {
        ...
        thickness: format.Art_Epaisseur,
        isPublic: format.isPublic,
        isPreorder: format.isPreorder,
        preorderMinArrivalDate: format.preorderMinArrivalDate,
        preorderMaxArrivalDate: format.preorderMaxArrivalDate,
    ...
    const responseProduct = {
      ...
      preorderMinArrivalDate: product.preorderMinArrivalDate,
      preorderMaxArrivalDate: product.preorderMaxArrivalDate,
      formatType: product.formatType,
...
const getProducts = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const products: any[] = await prisma.$queryRawUnsafe(`
      SELECT p.id,
             ...
             p.isPublic,
             p.isPreorder,
             p.preorderMinArrivalDate,
             p.preorderMaxArrivalDate,
             p.Art_Ean13,
             p.Art_Titre,
             p.Art_Image_Url,
             p.Art_Prix,
    ...
    //I need this for doing aggregatedCounts
    const totalProducts: any[] = await prisma.$queryRawUnsafe(`
      SELECT
        p.isPreorder,
        p.Art_DateParu,
        p.formatType AS mainFormatType,
        GROUP_CONCAT(formats.formatType) AS relatedFormats,
        CAST(
          (
            SELECT SUM(ps.Art_Stock)
            FROM ProductStock ps 
      ...
      FROM \`Product\` p
      LEFT JOIN \`Product\` formats ON formats.primaryFormatIsbn = p.Art_Ean13
      LEFT JOIN \`Category\` pc ON p.primaryCategoryNumber = pc.categoryNumber
      JOIN \`_CategoryProducts\` cp ON p.id = cp.B
      JOIN \`Category\` c ON c.id = cp.A
      LEFT JOIN \`CategoryTranslation\` ct ON ct.categoryNumber = c.categoryNumber
  ...
  `)
    ...
    inStockCount = totalProducts.filter((product) => product.stockCount > 0).length
    preorderCount = totalProducts.filter((product) => product.isPreorder == true).length
    publishedCount = totalProducts.filter(
      (product) => product.Art_DateParu !== null && new Date(product.Art_DateParu) <= new Date()
    ).length
    publishedPast6MonthsCount = totalProducts.filter((product) => {
      if (product.Art_DateParu) {
        const publishDate = new Date(product.Art_DateParu)
        const sixMonthsAgo = new Date()
        sixMonthsAgo.setMonth(sixMonthsAgo.getMonth() - 6)

        return publishDate >= sixMonthsAgo && publishDate <= new Date()
      }
      return false
    }).length
    // Assuming totalProducts is the result of your query
    publishedWithin3MonthsCount = totalProducts.filter((product) => {
      if (product.Art_DateParu) {
        const publishDate = new Date(product.Art_DateParu)
        const now = new Date()
        ...
        return publishDate >= now && publishDate <= threeMonthsLater
      }
      return false
    }).length
    paperbackFormatCount = totalProducts.filter((product) => {
      const relatedFormats = product.relatedFormats ? product.relatedFormats.split(',') : []
      return product.mainFormatType === 'paperback' || relatedFormats.includes('paperback')
    }).length

    grandFormatCount = totalProducts.filter((product) => {
      const relatedFormats = product.relatedFormats ? product.relatedFormats.split(',') : []
      return product.mainFormatType === 'grand' || relatedFormats.includes('grand')
    }).length
